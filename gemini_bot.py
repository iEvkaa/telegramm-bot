...
VERSION = """
Update 1.0.1
- Теперь обработка кнопок происходит в новом сообщении (видно старое сообщение, оно не редактируется)
- Оптимизированы некоторые функции. Убрана избыточность базы в функциях добавления и удаления юзеров
- Оптимизирована разметка Markdown. Тесты пройдены. Ошибок не обнаружено
- Добавлена кнопка для информации о боте
- Добавлена кнопка для просмотра текущей версии
- спизжено у ии новая структура бота
- зарабаотали "улучшения"

Предстоящие доработки в новых версиях:
- Научить бота запоминать и напоминать в будущем
- Научить бота писать пользователям, например, просить налить кофе
- Добавить кнопку с описанием функционала бота
- Убрать приведение username к нижнему регистру для безопасности
- Подумать о бэкапах и системе контроля версий
- Подумать о следующих улучшениях
"""

INFORMATION_ABOUT_BOT = """
Это бот с искусственным интеллектом
Интеллектуальная собственность Ивана
...
#----- Жена застовляет называть ее Кефирка((((--------------------------------------
